...
public class Recipe implements Statement {
    ...
    /**
     * To run the default recipe from a provided cookbook with the specified
     * name.
     * 
     * @param cookbook
     */
    public Recipe(String cookbook) {
        this(cookbook, null, null);
    }
    ...
    public Recipe(String cookbook, String recipe) {
        this(cookbook, recipe, null);
    }

    /**
     * To run a particular recipe from a cookbook located at <code>url</code>.
     * To use the default recipe pass <code>recipe</code> as <code>null</code>
     * 
     * @param cookbook
     * @param recipe
     * @param url
    ...
    public Recipe(String cookbook, String recipe, String url) {
        ...
        this.recipe = recipe;
        this.url = url;
        fileName = cookbook + "::" + recipe + "-" + System.currentTimeMillis();
    }
    ...
    /**
     * Transforms the recipe into the series of statements that must be ran on
     * the server (after chef is installed).
     * 
     * @return
     */
    @Override
    public String render(OsFamily family) {
        ...
        if (family.equals(OsFamily.UNIX)) {
            ...
            Statement storeJSonFile = Statements.exec("echo '" + toJSON()
                    + "' >> " + fileName);
            ...
            return Statements.newStatementList(storeJSonFile, runRecipe)
                    .render(family);
        } else {
